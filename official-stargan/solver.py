...
class Solver(object):
    ...
    def test_and_save(self, single_image):
        """Translate images using StarGAN trained on a single dataset. Additioanly, save the generated images"""
        # Load the trained generator.
        self.restore_model(self.test_iters)
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                # Get image filename
                filename = data_loader.dataset.get_image_name(i).split(".")[0]
                ...
                x_fake_list = torch.stack(x_fake_list).cpu()
                single_images = x_fake_list

                # Create a folder for each image to save
                result_path = os.path.join(self.result_dir, f"{filename}")
                os.makedirs(result_path, exist_ok=True)

                for i, img in enumerate(self.denorm(x_fake_list.cpu())):
                    save_image(img, f"{result_path}/{filename}_{i}.png")
                print(f'Saved real and fake images for {filename} into {self.result_dir}.')
